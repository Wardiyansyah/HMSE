...
const Home: NextPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      {/* Login button halaman utama(gajadi)
      <div className="fixed bottom-6 left-6 z-50">
        <Link href="/login">
          <Button className="bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-full shadow-lg">Masuk</Button>
        </Link>
      </div> */}
      {/* Chatbot */}
        ...
        {chatOpen && (
          <div className="mt-2 w-80 p-4 bg-white rounded-xl shadow-xl max-h-[500px] overflow-y-auto">
            ...
            <div className="flex items-center gap-2">
              <input className="w-full border px-2 py-1 rounded text-sm" value={input} placeholder="Tulis pesan..." onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} />
              <Button size="sm" onClick={handleSend} className="bg-blue-500 text-white">
                Kirim
              </Button>
            </div>
          </div>
